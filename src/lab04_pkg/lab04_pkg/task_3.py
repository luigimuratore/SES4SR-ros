...
class Task_3(Node):  # Node for EKF-based robot localization using real camera landmarks
    def __init__(self):
        ...
        self.get_logger().info(f'EKF initialized at [{initial_x:.2f}, {initial_y:.2f}, {initial_theta:.2f}]')
        ...
        # Subscribers - MODIFIED for real robot: using /camera/landmarks
        self.odom_sub = self.create_subscription(Odometry, '/odom', self.odom_callback, 10)
        self.landmark_sub = self.create_subscription(
            LandmarkArray, '/camera/landmarks', self.landmark_callback, 10)
        ...
        self.get_logger().info('EKF Localization Node (Task 3 - Real Robot) initialized')
        self.get_logger().info(f'Prediction rate: {self.prediction_rate} Hz')
        self.get_logger().info('Subscribed to /camera/landmarks for real AprilTag detection')
    ...
    def range_bearing_to_xy(self, robot_x, robot_y, robot_theta, range_val, bearing):
        """
        Transform range and bearing measurements to X-Y coordinates in odom frame.
        
        Args:
            robot_x: Robot x position in odom frame
            robot_y: Robot y position in odom frame
            robot_theta: Robot orientation in odom frame
            range_val: Range measurement from robot to landmark
            bearing: Bearing measurement relative to robot's heading
            
        Returns:
            tuple: (landmark_x, landmark_y) in odom frame
        """
        # Landmark position in odom frame
    ...
    def landmark_callback(self, msg: LandmarkArray):
        ...
        for lm in msg.landmarks:
            ...
            # Get known landmark position from map
            lm_x, lm_y = self.landmarks[lm_id]
            
            # Measurement from camera: range and bearing
            z = np.array([lm.range, lm.bearing])

            # Transform measurement to X-Y coordinates in odom frame for visualization/debugging
            detected_x, detected_y = self.range_bearing_to_xy(
                self.ekf.mu[0], self.ekf.mu[1], self.ekf.mu[2],
                lm.range, lm.bearing
            )
            ...
            # EKF Update using the known landmark position
            self.ekf.update(
                z=z,
                eval_hx=lambda x, y, th, lx, ly: landmark_range_bearing_model(
                    np.array([x, y, th]), np.array([lx, ly]), sigma=[0.0, 0.0]),
                eval_Ht=lambda x, y, th, lx, ly: compute_jacobians(
                    np.array([x, y, th]), np.array([lx, ly])),
                Qt=self.Qt,
            ...
            self.get_logger().info(
                ...
                f'detected XY in odom = ({detected_x:.2f}, {detected_y:.2f}), '
                f'Δμ = [{dmu[0]:.3f}, {dmu[1]:.3f}, {np.degrees(dmu[2]):.1f}°], '
                f'σ red ≈ [{sigma_red[0]:.3f}, {sigma_red[1]:.3f}]')

            # Publish goal pose for each detected landmark
            self._publish_goal_pose(detected_x, detected_y)
    ...
    def _publish_goal_pose(self, x, y):
        """Publish detected landmark position as goal pose in odom frame."""
        msg = PoseStamped()
        msg.header.stamp = self.get_clock().now().to_msg()
        msg.header.frame_id = 'odom'
